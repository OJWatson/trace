...
def casualty_model(
    ...
    # Optional: Overdispersion parameters for Negative Binomial
    # phi_hosp = numpyro.sample("phi_hosp", dist.Gamma(0.5, 0.5))
    # phi_death = numpyro.sample("phi_death", dist.Gamma(0.5, 0.5))
    ...
    obs_inj = numpyro.sample(
        "obs_injuries", dist.Poisson(lam_injuries).to_event(2), obs=injuries_obs
    )
